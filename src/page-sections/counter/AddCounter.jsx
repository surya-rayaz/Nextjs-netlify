...
// -------------------------------------------------------------------
const AddCounter = ({ open, setOpen, response,checktype }) => {
  ...
  return <StyledAppModal open={open} handleClose={() => setOpen(false)}>
    <Box mb={2}>
      <H6 mb={1}>Add Counter</H6>
      <AppTextField onChange={(e) => setcoutername(e.target.value)} fullWidth size="small" placeholder="Counter name" />
    ...
    <FlexBox mt={4} gap={2}>
      <Button variant="contained" fullWidth onClick={() => CounterAdd()}>
        Add
      </Button>
